...
React Compiler automates much of the memoization work in compiled code, reducing the need for manual `useMemo`, `useCallback`, and `React.memo`. This changes the optimization landscape for state management: derived values, callback references, and component memoization often need fewer explicit hooks. For projects that adopt the compiler, the primary design concern remains choosing the right state location: local, URL, server, or global. The compiler can help with memoization mechanics; the developer still owns state boundaries and placement.
