...
GC is generational because most objects die young. Gen 0 collections are frequent and cheap; Gen 2 collections are infrequent and scan the entire heap. The LOH stores objects at or above approximately 85 KB and is collected with Gen 2. Pinned objects create fragmentation hazards and should be freed promptly in allocation-heavy paths. Server GC creates per-processor heaps for throughput; Workstation GC uses a single heap for lower overhead. Container memory limits directly affect GC triggering thresholds and heap sizing — a 512 MB container triggers collections earlier than a 16 GB VM.
...
Reflection reads assembly metadata at runtime, enabling frameworks to inspect types, create instances, and consume attributes. Its cost is real: uncached `PropertyInfo.GetValue` is approximately 100× slower than direct access. Source generators shift work to build time, producing code that is trimmable, AOT-compatible, and performance-equivalent to hand-written accessors.
