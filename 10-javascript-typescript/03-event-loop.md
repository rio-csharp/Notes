...
- Chunking CPU-heavy work with `setTimeout` or `scheduler.postTask` (as shown in the chunking example above). Note: `scheduler.postTask` is not supported in Safari -- prefer `setTimeout` for cross-browser chunking or use the [scheduler-polyfill](https://github.com/GoogleChromeLabs/scheduler-polyfill).
